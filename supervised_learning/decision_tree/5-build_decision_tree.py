...
class Node:
    ...
    def update_indicator(self):
        """
        Update the indicator function for this node based on its
        lower and upper bounds.
        """

        def is_large_enough(x):
            """
            Check if all features are above the lower bounds.
            """
            if not self.lower:
                return np.ones(x.shape[0], dtype=bool)
            conditions = [np.greater_equal(x[:, key], self.lower[key])
                          for key in self.lower]
            return np.all(conditions, axis=0)

        def is_small_enough(x):
            """
            Check if all features are below or equal to the upper bounds.
            """
            if not self.upper:
                return np.ones(x.shape[0], dtype=bool)
            conditions = [np.less_equal(x[:, key], self.upper[key])
